...
import { setItem } from "../redux/slice/shopSlice";
const CartItem = ({ click,countclick, data: { id, image, name, description, price },count }) => {
    ...
    function debounce(val = 0, wait = 1800) {
        ...
        timeout = setTimeout(() => {
            clearTimeout(timeout);
            dispatch(setItem({id:id,count:countitem + val}))
    ...
    return (
        <>
            <div id="cartitem" className="white">
                ...
                <div id="cartbtndiv" >
                    <button className="decrement" onClick={() => { if(countitem>0){setCount(countitem - 1); debounce(-1);} }}>-</button>
                    <span>{countitem}</span>
