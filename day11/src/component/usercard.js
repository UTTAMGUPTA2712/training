...
const UserCard = ({ data, curentChatRoomId,lastchat }) => {
    ...
    return (<>
        <div id="usercard"><img className={(statusnow) ? "online" : "offline"} src={data?.photo ?? avataricon} /><div><h1>{data?.name ?? "name"}</h1><h2>{(typing == true) ? <span>typing...</span> : lastchat}</h2></div></div>
    </>)
}
